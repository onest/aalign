...
#include "Timing.h"
using namespace std;
void align_db(const char *queryStr, int queryNum, const char *dbStrs, int *dbOffset, int dbNum, int *output, int numThread);
int main(int argc, char** argv)
{
    ...
    int *output = (int *)malloc(sizeof(int)*dbNum);
    ...
    for(int i = 0; i < 10; i++)
    {
        int *it = max_element(output, output+dbNum);
        if(*it != INT_MIN)
        {
            cout << "score " << *it << ": " << name[ind[it - output]] << endl;
            output[it-output] = INT_MIN;
        }
    }
    // for(int i =0; i < dbNum; i++)
    // {
