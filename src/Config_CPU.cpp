...
{
    int segLen = size/AVX2_WIDTH;
    __m256i vecGap  = _mm256_set1_epi32(_gap);
    __m256i vecScan = _mm256_load_si256((__m256i *)input);
            vecScan = _mm256_add_epi32(vecScan, vecGap);
    for(int i = 1; i < segLen; i++)
    {
        __m256i vecH = _mm256_load_si256((__m256i *)(input+i*AVX2_WIDTH));
    ...
    _mm256_store_si256((__m256i *)output, vecLast);
    for(int i = 1; i < segLen; i++)
    {
        __m256i vecH = _mm256_load_si256((__m256i *)(input+(i-1)*AVX2_WIDTH));
                vecH = _mm256_add_epi32(vecH, vecGap);
        vecLast= _mm256_max_epi32(vecH, _mm256_add_epi32(vecLast, vecGap));
        _mm256_store_si256((__m256i *)(output+i*AVX2_WIDTH), vecLast);
    }
}
...
void striped_vec_scan_min(int *input, int *output, int size, int _gap, int row_i)
{
    ...
    for(int i = 1; i < segLen; i++)
    {
        __m256i vecH = _mm256_load_si256((__m256i *)(input+i*AVX2_WIDTH));
                vecH = _mm256_add_epi32(vecH, vecGap);
        vecScan = _mm256_min_epi32(vecH, _mm256_add_epi32(vecScan, vecGap));
    }
    ...
    for(int i = 1; i < segLen; i++)
    {
        __m256i vecH = _mm256_load_si256((__m256i *)(input+(i-1)*AVX2_WIDTH));
                vecH = _mm256_add_epi32(vecH, vecGap);
        vecLast= _mm256_min_epi32(vecH, _mm256_add_epi32(vecLast, vecGap));
        _mm256_store_si256((__m256i *)(output+i*AVX2_WIDTH), vecLast);
    }
}
